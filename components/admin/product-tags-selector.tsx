...
import { Tag, Sparkles, TrendingUp, Star, Percent, Crown, Gem, Leaf, ShieldCheck, Truck, Flag, Plus, Loader2, User, AlertCircle, Trash2 } from 'lucide-react'
...
const DEFAULT_ICONS = [
  ...
  { name: 'check-circle', label: 'Галочка' },
  { name: 'flame', label: 'Огонь' },
  { name: 'badge-check', label: 'Значок' },
...
export function ProductTagsSelector({ productId, onChange }: ProductTagsSelectorProps) {
  ...
  const handleTagToggle = async (tag: ProductTag, checked: boolean) => {
    ...
    if (tag.product_id && tag.product_id === parseInt(productId.toString())) {
      if (!checked) {
        toast.info('Личный тег нельзя отвязать от товара. Для удаления используйте кнопку удаления.')
      }
      return
    }
    ...
    try {
      if (checked) {
        ...
        if (data.success) {
          ...
          toast.success(`Тег "${tag.name}" удален`)
        } else {
      ...
      toast.error('Ошибка изменения тега')
    }
  }
  ...
  const handleDeletePersonalTag = async (tag: ProductTag) => {
    if (!tag.product_id || !confirm(`Удалить личный тег "${tag.name}"?`)) {
      return
    }

    try {
      const response = await fetch(`/api/products/${productId}/personal-tags/${tag.id}`, {
        method: 'DELETE',
        credentials: 'include'
      })
  ...
  const handleCreateTag = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Обновляем список тегов
        await fetchAllTags()
        // Обновляем список выбранных тегов товара
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Теги товара</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <>
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="flex-1">
            <CardTitle className="flex items-center gap-2">
              Теги товара
              {allTags.filter(tag => tag.product_id).length > 0 && (
                <Badge variant="secondary" className="text-xs">
                  {allTags.filter(tag => tag.product_id).length} личных
                </Badge>
              )}
            </CardTitle>
            <CardDescription>
              Добавьте общие теги или создайте личные для этого товара
            </CardDescription>
          </div>
          <Button
            variant="outline"
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          {(() => {
            const personalTags = allTags.filter(tag => tag.product_id)
            const commonTags = allTags.filter(tag => !tag.product_id)
              ...
              <div className="space-y-4">
                ...
                {personalTags.length > 0 && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-700 mb-2">
                      Личные теги (только для этого товара)
                    </h4>
                ...
                {personalTags.length === 0 && (
                  <Alert>
                    ...
                    <AlertDescription>
                      <strong>Создайте личные теги для этого товара!</strong>
                      <br />
    ...
    <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Создать личный тег</DialogTitle>
          <DialogDescription>
            Этот тег будет доступен только для данного товара
          </DialogDescription>
        </DialogHeader>
